...
import './TopicAboutCard.css';

const TopicAboutCard = ({topic, followerData}) => {
    ...
    return (
        <div className="topic-about-card">
            <header className="title-section">About <Link to={`/topic/${topic.name}`} className="topic-name">{topic.name}</Link></header>
            <main className="body-section">
                <p className="description">{topic.description}</p>
                <div className="follower-section">
                    <div className="follower-stat">
                        <div className="follower-num">{shortify(followerData.followerCount)}</div>
                        <div className="follower-label">{pluralize(followerData.followerCount, 'follower')}</div>
                    </div>
                    <button type="button" onClick={followerData.toggleFollow} className="follow-btn btn">{followerData.isFollowed? 'Unfollow': 'Follow'}</button>
                </div>
                ...
                <div className="create-post-btn-div">
                    <button type="button" onClick={loginRequired(() => history.push(`/post/t/${topic.name}/create`))} className="create-post-btn btn">Create Post</button>
                </div>
            </main>
        </div>
     );
