...
const UserCard = ({user}) => {
    const dayJsDate = dayjs(user && user.date_joined);
    const date = dayJsDate.format('MMM D, YYYY');
    const {isFollowed, followerCount, toggleFollow} = useFollowHandler(user, 'user');
    
    return user && (
        <div className="user-card card-shadow">
            <header className="header-section">
                <img src={user.profile_pic} alt="" className="circle-profile-pic"/>
                <Link to={`/user/${user.username}`} className="username">u/{user.username}</Link>
            </header>
